...
def draw_steps_figure(config_list, fig_name):
    if len(config_list) == 2:
        ...
        crps_lstm_aq, crps_steps_lstm_aq, mre_lstm_aq, naps_lstm_aq, naps_steps_lstm_aq = get_prob_metrics(
            lstm_aq_exp_name)

        qsqf_c_exp_name = config_list[1][0]
        crps_qsqf_c, crps_steps_qsqf_c, mre_qsqf_c, naps_qsqf_c, naps_steps_qsqf_c = get_prob_metrics(qsqf_c_exp_name)
        ...
        plt.clf()
        plt.plot(x_data, naps_steps_lstm_aq[1:], 'bo-', alpha=0.5, linewidth=1, label=config_list[0][1])
        plt.plot(x_data, naps_steps_qsqf_c[1:], 'ro-', alpha=0.5, linewidth=1, label=config_list[1][1])

        plt.legend()
        plt.xlabel('Prediction Step')
        plt.ylabel('NAPS')
        plt.savefig(os.path.join(output_dir, f'NAPS {fig_name}.png'))
    elif len(config_list) == 1:
        exp_name = config_list[0][0]
        crps, crps_steps, mre, naps, naps_steps = get_prob_metrics(exp_name)
        ...
        plt.clf()
        plt.plot(x_data, naps_steps[1:], 'bo-', alpha=0.5, linewidth=1, label=config_list[0][1])

...
class Net(nn.Module):
    def __init__(self, params, device):
        ...
        self.pre_gamma = nn.Linear(
            params.lstm_hidden_dim * params.lstm_layers, params.num_spline - 1)
    ...
    def forward(self, x, hidden, cell):
        ...
        return ((beta_n1, sigma, torch.squeeze(gamma)), hidden, cell)
    ...
    def predict(self, x, hidden, cell, sampling=False):
        ...
        for j in range(self.params.sample_times):
            ...
            for t in range(self.params.pred_steps):
                ...
                beta_n1, sigma, gamma = func_param
                ...
                beta_N = torch.cat((beta_n1, gamma[:, :1]), dim=1)
                gamma_pad = pad(gamma, (1, -1))
                gamma_pad[:, 0] = gamma[:, 0]
                beta = (gamma - gamma_pad) / (2 * sigma[:, :-1])
                beta = pad(beta, (0, 1)) - pad(beta, (1, 0))

                ksi = pad(torch.cumsum(sigma, dim=1), (1, -1))
...
def loss_fn(func_param, labels: Variable):
    ...
    knots = pad(knots.T, (1, -1))  # F(ksi_1~K)=0~max
